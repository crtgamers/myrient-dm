<template>
  <!-- Resultados de búsqueda (en Favoritos la búsqueda ya está limitada a favoritos) -->
  <ErrorBoundary
  ...
  <!-- Contenido de carpeta favorita (dentro de una carpeta desde Favoritos) -->
  <ErrorBoundary
    v-else-if="showFolderContent"
    key="favorites-folder"
    component-name="FavoritesFolderContent"
  ...
  >
    ...
    >
      <FolderGrid
        v-if="navigationFolders.length > 0"
      ...
      <FileTable
        ...
        :is-at-root="isAtRoot"
        :has-active-filters="hasActiveFilters"
        :total-file-count="navigationFiles.length"
        :active-filter-count="activeFilterCount"
        @download="$emit('download', $event)"
      ...
      />
      <div
        v-if="hasMoreChildren && navigationFiles.length > 0"
          ...
          <span v-else>
            {{ t('explore.loadMore') }} ({{
              t('explore.loadMoreCount', {
                current: navigationFolders.length + navigationFiles.length,
      ...
      <div
        v-if="navigationFolders.length === 0 && navigationFiles.length === 0"
        class="empty-state"
        role="status"
        aria-live="polite"
      >
  ...
  <!-- Lista de carpetas favoritas (sin búsqueda activa ni carpeta abierta) -->
  <ErrorBoundary
    v-else
    key="favorites"
    component-name="FavoritesSection"
    :fallback-message="t('errors.loadFavorites')"
  >
...
import { Folder, File, Search, AlertTriangle, FolderOpen } from 'lucide-vue-next';
...
import type { FavoriteFolderItem } from '../components/FavoritesSection.vue';
...
const props = defineProps({
  ...
  showFolderContent: { type: Boolean, default: false },
  ...
  filteredSearchFoldersAsGrid: {
    type: Array as PropType<FolderGridItem[]>,
    default: () => [],
  },
  filteredSearchFilesAsTable: {
  ...
  favoriteIds: { type: Object as PropType<Set<number>>, default: () => new Set() },
  downloadsByFileId: { type: Object, default: () => ({}) },
  // Contenido de carpeta (cuando showFolderContent)
  navigationFolders: { type: Array as PropType<FolderGridItem[]>, default: () => [] },
  navigationFiles: { type: Array as PropType<FileTableItem[]>, default: () => [] },
  selectedFiles: { type: Array as PropType<number[]>, default: () => [] },
  isAllFilesSelected: { type: Boolean, default: false },
  currentNodeId: { type: Number, default: undefined },
  isAtRoot: { type: Boolean, default: false },
...
defineEmits<{
  (_e: 'navigate', _node: { id: number }): void;
  (_e: 'remove', _node: { id: number }): void;
